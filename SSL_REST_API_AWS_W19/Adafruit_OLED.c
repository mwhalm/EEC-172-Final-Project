...
*/
    ...
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0);  // DC Low
    GPIOPinWrite(GPIOA1_BASE, 0x20, 0);  // CS low
    SPIDataPut(GSPI_BASE ,c);
    SPIDataGet(GSPI_BASE, &dummy);
    GPIOPinWrite(GPIOA1_BASE, 0x20, 0xFF);  // CS High
...
*/
    unsigned long dummy;
    MAP_SPICSEnable(GSPI_BASE);
    GPIOPinWrite(GPIOA1_BASE, 0x10, 0xFF);
    GPIOPinWrite(GPIOA1_BASE, 0x20, 0);
    SPIDataPut(GSPI_BASE ,c);
    SPIDataGet(GSPI_BASE, &dummy);
    GPIOPinWrite(GPIOA1_BASE, 0x20, 0xFF);
    MAP_SPICSDisable(GSPI_BASE);
}
